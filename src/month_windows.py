...
for year in range(min(distinct_years),max(distinct_years)):
    ...
    # loop over months
    distinct_months = np.array(set(year_train_df.Month))
    for month in range(min(distinct_months), max(distinct_months)):
        # create subset of data of current year
